

import { Link } from 'react-router-dom'
import { useEffect, useState } from 'react';
import { Product } from '../models/Product';
import Chip from './Chip';
import { useAuth } from '../contexts/AuthContext';
import img from '../assets/rubbish-bin.svg'

function ProductCard({ product, handleOnRemove }: { product: Product, handleOnRemove:(productID:number)=>void }) {
    const IMAGE_BASE64 = product.image ? product.image : "data:image/png`;base64,"
    const [visible, setVisible] = useState(' opacity-0 ')
    const [avgRate, setAvgRate] = useState<number>(0)
    const [crudVisible, setCrudVisible] = useState(false)
    const auth = useAuth()

   



    useEffect(() => {
        const total = product.rates?.reduce((acu, rate) => rate.value + acu, 0)
        const avg = product.rates && total && total / product.rates?.length
        setAvgRate(avg || 0)
        setTimeout(() => setVisible(' opacity-100 '), 200)
    }, [])

    const handleOnClickRemove = ()=>{
        if(!product.id) return
        handleOnRemove(product.id)
    }


    const imagen = <img className='p-8 rounded-t-lg w-full h-full object-contain' src={IMAGE_BASE64} alt="" />

    return <>
        <div className={`w-full max-w-sm bg-white/90 border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800/90 dark:border-gray-700/90 ${visible} transition-opacity duration-1000 bg-opacity-50`} onMouseOver={() => auth.isAdmin && setCrudVisible(true)} onMouseOut={() => auth.isAdmin && setCrudVisible(false)}>
            <Link to={`/products/${product.id}`} >
                <div className='flex flex-row place-content-center'>
                    {product.categoryProduct?.map((c, index) => <Chip key={index} color={c.category.color}>{c.category.name}</Chip>)}
                </div>
                <div>
                    {imagen}
                </div>
                <div className="px-5 pb-5">
                    <div>
                        <h5 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">{product.name}</h5>
                    </div>
                    <div className="flex items-center mt-2.5 mb-5">
                        <div className="flex items-center space-x-1 rtl:space-x-reverse">
                            {Array.from({ length: 5 }, (_, i) => <svg key={i} className={`w-4 h-4 ${(i + 1 > Math.round(avgRate)) ? "text-gray-300" : "text-yellow-300"} hover:text-yellow-500 `} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                                <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z" />
                            </svg>)}
                        </div>
                        <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded-sm dark:bg-blue-200 dark:text-blue-800 ms-3">{avgRate}</span>
                    </div>
                    <div className="flex flex-row items-center justify-between text-3xl lg:text-xl">
                        <span className="font-bold text-gray-900 dark:text-white">{(product.price * (1 - product.discount / 100)).toFixed(2).replace(".", ",")}€{product.discount ? <span className='ms-3 text-gray-500 line-through font-medium'>{product.price.toFixed(2).replace(".", ",")}€</span> : ""}</span>
                        {product.discount ? <span className='rounded-lg bg-green-800 font-bold p-2'>{product.discount}%</span> : ""}
                    </div>
                </div>
            </Link>
            {crudVisible && <div className="flex flex-row items-center justify-end gap-x-4 text-3xl lg:text-xl ">
                <Link to={`${product.id}/update`} className='rounded-2xl border-black cursor-pointer'>
                    Edit
                </Link>
                <button type='button' onClick={handleOnClickRemove} className='rounded-2xl border-black cursor-pointer'>
                    <img src={img} alt="Remove" className='w-6 ' />
                </button>
            </div>
            }
        </div>
    </>
}

export default ProductCard