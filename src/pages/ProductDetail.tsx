...
function ProductDetail() {
  ...
  function calcFinalPrice() {
    if (!product?.discount) return setFinalPrice(product?.price)
    if (!product?.price) return setFinalPrice(0)
    setFinalPrice(product?.price * (1 - product?.discount / 100))
  }
  const handleAdd = () => {
  ...
  const discountSpan = <span className='text-end bg-green-800 rounded-lg px-1 content-center font-bold'>{product?.discount}%</span>
